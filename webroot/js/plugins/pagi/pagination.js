...
var Pagination = function (el, options) {
	...
	var $container = $(el),
		...
		allPagesLabel = options.labels.all || 'すべて',
	...
	callBack = function(){
		if (typeof options.showPage === 'function') {
			options.showPage(currentPage);
		}
	};
	...
	getRenderedPageRange = function () {
		var result = '<div class="text-right1"><label class="label fas fa-chevron-down "><select class="page-range form-control form-control-borderless w-auto ml-5">';

		$.each(pageRange, function (i, p) {
			var selected = pageSize === p ? 'selected="selected"' : '';
			result += '<option ' + selected + ' value="' + p + '">' + (p === ALL ? allPagesLabel : p+'件') + '</option>';
		});

		return result += '</select></label></div>';
	};
